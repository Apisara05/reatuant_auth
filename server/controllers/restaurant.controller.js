...
restaurantController.getAll = async (req, res) => {
  try {
    await Restaurant.findAll().then((rest) => {
      res.send(rest);
    });
  } catch (e) {
    res.status(500).send({
...
restaurantController.getById = async (req, res) => {
  try {
    ...
    res.status(500).send({
      message:
        error.message || "Something error while getting restaurant with id",
    });
  }
};
    ...
    await Restaurant.update(
      ...
      if (n[0] === 1) {
        ...
        res
          .status(404)
          .send({
            message: `Can not update restaurant with id ${id}. Maybe restaurant was not found or req. body is emety!`,
          });
      }
    });
  } catch (e) {
      ...
      .then((n) => {
        if (n === 1) {
          ...
          res
            .status(404)
            .send({
              message: `Can not deleted restaurant with id ${id}. Maybe restaurant was not found or req. body is emety!`,
            });
        }
      })
      .catch((error) => {
